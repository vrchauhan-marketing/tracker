...
function createMcpServer() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (name === 'log_activity') {
            const { url, platform, activity_type, topic_tags, date_posted = new Date().toISOString().split('T')[0], title = null, notes = null, logged_by = null, } = args;
            try {
                db.prepare(`
          INSERT INTO activities (id, date_posted, platform, activity_type, url, title, topic_tags, notes, logged_by)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(randomUUID(), date_posted, platform, activity_type, url, title, JSON.stringify(topic_tags), notes, logged_by);
                return {
                    content: [
                        {
                            type: 'text',
